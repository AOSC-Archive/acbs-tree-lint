		    ...
		    if (line.attr === 'SRCTBL') {
                        const parseValStringRecursively = function (inputStr) {
                            return inputStr.replace(/\$\w+/g, function () {
                                ...
                                let end = arguments[3];
                                let val = parsedContent.filter(x => x.attr === varname)[0].val.replace(/\"/g, '').slice(start, end);
                                return val;
                            }).replace(/\$\{(\w+)\/(.+?)\/(.+?)\}/g, function () {
                                let varname = arguments[1];
                                let start = arguments[2].replace(/\\/g, '');
                                let end = arguments[3];
                                let val = parsedContent.filter(x => x.attr === varname)[0].val.replace(/\"/g, '').replace(start, end);
                                return val;
                            });
                        };
                        let statusCode = 0;
                        ...
                        const makeRequest = function (url) {
                            let protocol = url.toLowerCase().replace(/\:\/\/.+$/, '').replace(/[^\w]/g, '');
                        ...
                        i < 2**30 ? makeRequest(currentUrl) : 0;
                    };
                    // Conflict
